...
Method-level scheduling (per `Execute`, in `EOS.Attributes`):

- `[Delay(float seconds)]` / `[DelayFrame(int frames)]` — coroutine-like cadence: the `Execute` runs at the given interval, not every frame. Fires **immediately on the first eligible frame, then waits the interval**. Implemented as a run-level gate in `SystemsRunner.Run`: a sleeping frame skips the body **and does not advance the reactive cursor**, so `[New]`/`[Bumped]`/`[Enabled]`/`[Disabled]` accumulate and all fire on the next run (a disabled system still drops, as before — delay is a separate gate from enable/disable). Age-trimmed channels (`[Removed]`, events) are kept long enough via `World.ReactiveRetentionFrames` (below). Both attributes together → `[DelayFrame]` wins (warning). The whole feature is invisible to codegen — the body is unchanged.
- `[Budget(int maxPerFrame)]` — per-frame quota: at most `n` matched entities are processed per frame, the rest roll over to following frames (time-slicing), nothing is lost. Non-reactive queries round-robin a persistent offset (fair, bounded, eventually-complete; not a strict partition under structural churn). Reactive queries with a **single** reactive parameter (the common `[New] T` / `[Removed] T` case) process pending edges in ascending version order and commit the cursor only past what they consumed (deferred edges fire next frame). A query with **multiple** reactive parameters, or an interface-driven reactive query, logs a warning and runs **without** a budget (to avoid dropping an edge). `[Budget]` always runs on the reflection path (`SystemShape.CanTypeBody` returns false for it) — codegen is a perf optimization and budgeted bodies are throttled-heavy, so the boxing cost is negligible.
...
Lifecycle each phase: `Promote` moves staging → live with ascending sequence numbers (events surface one tick after emit); `Trim` drops live events. **Read-once is guaranteed by a per-`EventExecute` cursor** (a sequence watermark, like `[New]`), so multi-tick `FixedUpdate` and phase ordering never double-read. Retirement is **min-cursor**: an event lives until every registered consumer of its type has advanced past it, then it is dropped. The hard age cap is `World.ReactiveRetentionFrames` (default `EventsContainer.MaxAge` = 16 frames, raised at init to cover the longest `[Delay]`/`[DelayFrame]`) so an undriven phase can't leak. A consumer in a disabled group still advances its cursor (events are dropped for it), consistent with the reactive caveat above.
